...
function PlayerAvatar({ player, index, isCurrentPlayer }) {
  ...
  return (
    <div className={`flex flex-col items-center gap-1.5 animate-scale-in ${!player.connected ? 'opacity-40' : ''}`}>
      <div
        className={`w-14 h-14 rounded-full bg-gradient-to-br ${colorClass} flex items-center justify-center text-white font-bold text-lg shadow-lg relative`}
      >
        {initials}
        {isCurrentPlayer && (
          <div className="absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full bg-neon-green border-2 border-dark-950" />
        )}
        {!player.connected && (
          <div className="absolute inset-0 rounded-full bg-dark-950/60 flex items-center justify-center">
            <span className="text-xs">📡</span>
          </div>
        )}
      </div>
      <span className="text-xs text-gray-300 font-medium truncate max-w-[70px] text-center">
        {player.name}
        {isCurrentPlayer && (
          <span className="text-[9px] text-gray-500 block">You</span>
        )}
      </span>
    </div>
  );
  ...
  return (
    <div className="w-full h-full flex flex-col px-6 py-8 overflow-y-auto">
      ...
      <div className="text-center mb-8 animate-slide-up">
        ...
        >
          <div className="text-5xl sm:text-6xl font-black tracking-[0.2em] text-white hover:text-accent transition-colors duration-200">
            {roomCode}
          </div>
          <div className={`mt-2 text-xs transition-all duration-200 ${copied ? 'text-neon-green' : 'text-gray-500 group-hover:text-gray-300'}`}>
        ...
        <div className="space-y-4 animate-slide-up" style={{ animationDelay: '0.2s' }}>
            ...
            <div className="grid grid-cols-3 gap-3">
              {CATEGORIES.map((cat) => (
                ...
                >
                  ...
                  <div className="text-[9px] text-gray-500">{cat.desc}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <button
            ...
            className={`w-full py-4 rounded-xl font-bold text-lg transition-all duration-300 active:scale-[0.98] ${
              canStart
                ? 'bg-gradient-to-r from-accent to-accent-light text-white shadow-lg shadow-accent/25 hover:shadow-xl hover:shadow-accent/30 animate-glow'
